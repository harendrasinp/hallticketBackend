...
const BASE_URL = "https://hallticketbackend.onrender.com"; // 🔴 CHANGE IF NEEDED
...
router.post("/generate-hallticket", async (req, res) => {
  try {
    ...
    doc.image(logo1, headerX, 45, { width: logoSize });
    doc.image(logo2, headerX + logoSize + gap + textWidth + gap, 45, { width: logoSize });

    const textX = headerX + logoSize + gap;

    doc.font("Helvetica-Bold").fontSize(20)
      .text("TAPI EDUCATION ACADEMY", textX, 45, { width: textWidth, align: "center" });

    doc.font("Helvetica-Bold").fontSize(15)
      .text("P.P SAVANI VIDHYAMANDIR", textX, 71, { width: textWidth, align: "center" });

    doc.font("Helvetica").fontSize(10)
      .text("AT POST KATHGADH VYARA, DIST. TAPI", textX, 91, { width: textWidth, align: "center" });
